...
select idSensor, temperatura, umidade from dadoSensor inner join Sensor on fkSensor = idSensor where fkCliente = 2;

select temperatura, umidade, DATE_FORMAT(dataDado,'%H:%i:%s') as momento_grafico from dadoSensor inner join Sensor on fkSensor = idSensor where idSensor = 1 order by idDado desc limit 3;
